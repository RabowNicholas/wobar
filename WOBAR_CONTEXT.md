...
| [[working/RESEARCH_PLAN_BRAND]] | /working | **The unrun research round for the brand-psychology sibling series — briefs ready to paste into subagents.** Parked deliberately: the WebSearch budget was exhausted by the storytelling round's failed Reddit agents, and WebFetch alone gives retrieval without discovery — which would produce a one-sided map of a contested field that *looks* complete. Carries the three locked decisions (**curator not judge · no teardowns yet · analysis not testimony**), the inverted source strategy (theory-primary, Reddit secondary — the reverse of last round), the seven method rules, and six briefs: archetypes and their evidence base · **the evidence war** (positioning/personality vs Ehrenberg-Bass, mapped fairly, including whether any of it applies to *small* brands) · brand voice · visual identity anchored on distinctive assets · competitive content landscape · exemplar map. §6 names the design problem research can't solve: whether *"digestible on social"* and *"the evidence is contested"* survive in the same piece | ready to run — blocked on budget |
