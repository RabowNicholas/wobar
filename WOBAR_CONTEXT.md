...
| [[working/WOBAR_SOCIAL_PLAN]] | /working | **GOVERNING for Instagram and TikTok. v2.0 (2026-08-12) reverses v1.0's central decision.** Instagram is now **a surface in its own right and growing it is a goal** — account strength *is* the credibility mechanism, and v1.0 banned that goal on evidence (spüke/ISEEU, Vulora) that only ever addressed listeners and label gates. **§0 is the safety rail: music first, always — IG runs on surplus, so *post more* is not an available recommendation.** §1 also assigns a job no surface in this vault had: **reading reaction to the music**, faster than SoundCloud can — it shapes release order, sound direction and where visual effort goes, and **never kills a sketch**. Audience is **lane-weighted** (growth is the goal; the account must still read as a producer's account) with target tier **one rung up (2K–20K)**. Three buckets — **music** (proof, capture-gated, legibility not brand), **memes for producers** (the reach mechanism, and CUTDWN's documented failure mode — pin the proof), **give-back** (broadcast deferred; **1:1 starts now**). Funnel: memes → engagement → **triage** → proof → **outbound warm DM** → **SoundCloud, not a smart link**. **§5 owns social measurement — the old do-not-track ban is lifted**, replaced by three tiers where **who outranks how many**, plus the ⚠ **inversion constraint** (IG growth only counts if SoundCloud moves with it) and the ⚠ **90-day window** that destroys account-level Insights if nobody captures it. **§1 carries the correction that the 316-send "distribution is not the constraint" finding was a Flow State collab and is void.** §7 keeps the failure-mode test — *does it cause an uncomfortable action, or license a comfortable one?* — and applies it to the biweekly review itself. Supersedes the Platforms table in WOBAR_CONTENT and §6 of WOBAR_GROWTH_PLAN | live (v2.0) |
